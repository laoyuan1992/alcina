...
half3 _Gravity;
half3 _Life;        // dt / max_life, dt / (max_life * speed_to_life), dt / min_life
...
float4 NewParticlePosition(float2 uv)
{
	...
	float3 jitter = float3(UVRandom(uv, _Time.x + 123.456),
		UVRandom(uv, _Time.x + 789.012),
		UVRandom(uv, _Time.x + 345.678));
	p += jitter * _PositionJitter;
...
float4 UpdatePositionFragment(v2f_img i) : SV_Target
{
    ...
    p.w -= min(max(_Life.x,  _Life.y / v.w), _Life.z) * rnd;
